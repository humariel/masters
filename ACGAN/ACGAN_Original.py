...
class ACGAN():
    ...
    def create_discriminator(self):
        ...
        #convolution
        D = layers.Conv2D(16, (3,3), strides=(2,2), padding='same', kernel_initializer='glorot_normal', bias_initializer='zeros')(in_image)
        D = layers.LeakyReLU(alpha=0.2)(D)
        D = layers.Dropout(0.5)(D)
        #convolution
        D = layers.Conv2D(32, (3,3), strides=(1,1), padding='same', kernel_initializer='glorot_normal', bias_initializer='zeros')(D)
        D = layers.BatchNormalization()(D)
        D = layers.LeakyReLU(alpha=0.2)(D)
        D = layers.Dropout(0.5)(D)
        #convolution
        D = layers.Conv2D(64, (3,3), strides=(2,2), padding='same', kernel_initializer='glorot_normal', bias_initializer='zeros')(D)
        D = layers.BatchNormalization()(D)
        D = layers.LeakyReLU(alpha=0.2)(D)
        D = layers.Dropout(0.5)(D)
        #convolution
        D = layers.Conv2D(128, (3,3), strides=(1,1), padding='same', kernel_initializer='glorot_normal', bias_initializer='zeros')(D)
        D = layers.BatchNormalization()(D)
        D = layers.LeakyReLU(alpha=0.2)(D)
        D = layers.Dropout(0.5)(D)
        #convolution
        D = layers.Conv2D(256, (3,3), strides=(2,2), padding='same', kernel_initializer='glorot_normal', bias_initializer='zeros')(D)
        D = layers.BatchNormalization()(D)
        D = layers.LeakyReLU(alpha=0.2)(D)
        D = layers.Dropout(0.5)(D)
        #convolution
        D = layers.Conv2D(512, (3,3), strides=(1,1), padding='same', kernel_initializer='glorot_normal', bias_initializer='zeros')(D)
        D = layers.BatchNormalization()(D)
        D = layers.LeakyReLU(alpha=0.2)(D)
        D = layers.Dropout(0.5)(D)
        #last layer
        D = layers.Flatten()(D)
        # real/fake output
        out1 = layers.Dense(1, activation='sigmoid', name='out_fake', kernel_initializer='glorot_normal', bias_initializer='zeros')(D)
        # class label output
        out2 = layers.Dense(self.n_classes, activation='softmax', name='out_aux', kernel_initializer='glorot_normal', bias_initializer='zeros')(D)
        # define model
    ...
    def generate_fake_samples(self, n_samples):
        ...
        # predict outputs
        images = self.generator([z_input, labels_input])
        # create class labels
    ...
    def train(self, train_set, val_set, test_set, batches_per_epoch, epochs=100, batch_size=32):
        ...
        for epoch in range(epochs):
            ...
            val_loss =  self.discriminator.evaluate(val_x, [np.ones((val_x.shape[0], 1)), val_y])
            #print('val loss: ', val_loss)
            test_loss =  self.discriminator.evaluate(test_x, [np.ones((test_x.shape[0], 1)), test_y])
