...
class BigACGAN():
    ...
    def create_discriminator(self):
        ...
        D = layers.Conv2D(64, 5, padding="same", kernel_initializer=init)(in_image)
        # ResBlocks
        D = ResnetBlockDown(D, 64)
        D = Attention(D, epsilon=1.0e-8)
        D = ResnetBlockDown(D, 128)
        D = ResnetBlockDown(D, 256)
        D = ResnetBlockDown(D, 512)
        D = ResnetBlockDown(D, 1024)
        D = ResnetBlock(D, 1024)
        D = layers.LeakyReLU(alpha=0.2)(D)
        D = GlobalSumPooling2D()(D)
        #last layer
        D = layers.Flatten()(D)
        # real/fake output
        out1 = layers.Dense(1, kernel_initializer=init, activation='sigmoid', name='out_fake')(D)
        # class label output
        out2 = layers.Dense(1, kernel_initializer=init, activation='sigmoid', name='out_aux')(D)
        # define model
        model = tf.keras.Model(in_image, [out1, out2], name="discriminator")
        # compile model
        opt = tf.keras.optimizers.Adam(lr=0.0004, beta_1=0, beta_2=0.9, epsilon=1.0e-8)
        model.compile(
    ...
    def create_generator(self):
        ...
        y_emb = tfa.layers.SpectralNormalization(layers.Dense(self.latent_dim, use_bias=False, kernel_initializer=init))(y)
        c = layers.Concatenate()([z, y_emb])

        G = tfa.layers.SpectralNormalization(layers.Dense(4*4*1024, use_bias=False, kernel_initializer=init))(c)
        G = layers.Reshape((4,4,1024))(G)
        # ResBlocks
        G = ResnetBlockUp(G, c, 1024)
        G = ResnetBlockUp(G, c, 512)
        G = ResnetBlockUp(G, c, 256)
        G = ResnetBlockUp(G, c, 128) 
        G = Attention(G, epsilon=1.0e-8)
        G = ResnetBlockUp(G, c, 64)
        # End part
        G = ConditionalBatchNormalization(G, c, momentum=0.99, epsilon=1.0e-8)
        G = layers.LeakyReLU(alpha=0.2)(G)
        G = layers.Conv2D(3, 3, padding="same", kernel_initializer=init)(G)
    ...
    def define_gan(self):
        ...
        # compile model
        opt = tf.keras.optimizers.Adam(lr=0.0001, beta_1=0, beta_2=0.9, epsilon=1.0e-8)
    ...
    def generate_real_samples(self, image_generator):
        ...
        if labels.shape[0] != image_generator.batch_size:
            X,labels = image_generator.next()

        labels = labels.reshape((image_generator.batch_size,1))
        # generate class labels
        y = np.ones((image_generator.batch_size, 1))
        return [X, labels], y
    ...
    def train(self, train_gen, val_gen, batches_per_epoch, epochs=100, batch_size=32):
        ...
        for epoch in range(epochs):
            ...
            val_loss =  self.discriminator.evaluate(val_x, [np.ones((val_gen.batch_size, 1)), val_y], batch_size=batch_size, return_dict=True)
            ...
            if (epoch + 1) % 10 == 0 or (epoch + 1) < 10:
                ...
                latent_points, labels = self.generate_latent_points_all_classes(16)
